<script lang="ts">
  ...
  let budget = todo.budget || 0;
  let goalNote = todo.goal_note;
  let notes = todo.notes;
  let repeatType = todo.schedule.repeat.type;
  let startDate = todo.schedule.repeat.start_date;
  let tasks = [...todo.tasks.map(task => ({
    ...
    unit: task.unit || '',
    code: task.code || '',
    financeLinked: task.finance?.linked || false,
    financePrice: task.finance?.price || 0,
    done: task.done,
    attachments: task.attachments?.map(att => ({
      ...att,
      qrText: '',
      showPreview: true,
      originalImage: att.type === 'qr_code' || att.type === 'photo' ? att.url : '',
      extractedText: att.type === 'qr_code' ? 'Sample QR Text' : ''
    })) || [],
  ...
  function addTask() {
    tasks = [...tasks, { title: '', time: '', quantity: 0, unit: '', code: '', financeLinked: false, financePrice: 0, done: false, attachments: [], showAttachments: false }];
  }
  ...
  function saveTodo() {
    ...
    todos.update(todoList => 
      todoList.map(t => t.id === todo.id ? updatedTodo : t)
    );
    dispatch('close');
  }

  function deleteTodo() {
    todos.update(todoList => todoList.filter(t => t.id !== todo.id));
    dispatch('close');
  }
...
<div class="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center p-0 md:p-4 z-50">
  <div class="bg-white rounded-t-2xl md:rounded-2xl shadow-2xl w-full md:max-w-4xl h-[95vh] md:h-[90vh] flex flex-col">
    ...
    <div class="flex items-center justify-between p-6 border-b border-gray-100 bg-blue-50 flex-shrink-0">
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <p class="text-sm text-gray-600">Ubah detail dan atur ulang aktivitas</p>
        </div>
      </div>
      <button 
        on:click={closeModal}
    ...
    <!-- Progress Indicator -->
    {#if tasks.length > 0}
      <div class="px-6 py-4 bg-green-50 border-b border-gray-100 flex-shrink-0">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-medium text-gray-700">Progress Task</span>
          <span class="text-sm font-bold text-gray-900">{getProgressPercentage()}%</span>
        </div>
        <div class="progress-bar">
          <div class="progress-fill" style="width: {getProgressPercentage()}%"></div>
        </div>
        <div class="flex items-center justify-between mt-2 text-xs text-gray-600">
          <span>{getCompletedTasksCount()} dari {tasks.length} tugas selesai</span>
          <span>Dibuat: {new Date(todo.created_at).toLocaleDateString('id-ID')}</span>
        </div>
      </div>
    {/if}
              ...
              <div class="space-y-2">
                {#each priorities as prio}
                  ...
                  >
                    ...
                    <div class="priority-dot {prio.value === 'high' ? 'bg-red-500' : prio.value === 'medium' ? 'bg-amber-500' : 'bg-green-500'}"></div>
                  </button>
                {/each}
              </div>
            </div>
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-4">
            {#each tasks as task, index}
              <div class="bg-gray-50 rounded-lg p-4 space-y-4 {task.done ? 'opacity-75' : ''}">
                <div class="flex items-start space-x-3">
                  <button 
                    on:click={() => task.done = !task.done}
                    class="task-checkbox {task.done ? 'checked' : 'unchecked'} mt-2"
                  >
                    {#if task.done}
                  ...
                  <div class="flex-1">
                    <input 
                      ...
                      class="form-input {task.done ? 'line-through text-gray-500 bg-gray-100' : ''}"
                            ...
                            {#if attachment.url && attachment.showPreview}
                              ...
                              <div class="flex space-x-2 justify-center">
                                {#if attachment.extractedText}
                                  <button 
                                    on:click={() => regenerateQRFromImage(index, attachmentIndex)}
                                ...
                                {#if attachment.originalImage}
                                  <button 
                                    on:click={() => restoreOriginalImage(index, attachmentIndex)}
...
{#if showDeleteConfirm}
  <div class="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-60">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
      <div class="flex items-center space-x-3 mb-4">
        ...
        <div>
          <h3 class="text-lg font-bold text-gray-900">Hapus Task</h3>
          <p class="text-sm text-gray-600">Tindakan ini tidak dapat dibatalkan</p>
        </div>
      </div>
      
      <p class="text-gray-700 mb-6">
        Apakah Anda yakin ingin menghapus task "<strong>{title}</strong>"? 
        Semua data dan progress akan hilang secara permanen.
      </p>
